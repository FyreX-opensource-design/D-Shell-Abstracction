#!/bin/bash
# Wrapper script to run USB connect script as logged-in user
...
loginctl list-sessions --no-legend 2>/dev/null | while read -r _ _ user _; do
    if [ -n "$user" ] && [ "$user" != "tty" ]; then
        ...
        if [ -n "$uid" ]; then
            ...
            runuser -l "$user" -c "export XDG_RUNTIME_DIR=\"$runtime_dir\" DISPLAY=\":0\" PULSE_RUNTIME_PATH=\"$runtime_dir/pulse\"; /opt/system/lib/play-usb-connect-sound.sh" 2>/dev/null &
        fi
    fi
done
